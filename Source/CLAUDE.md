...
11. **無名名前空間（`namespace { }`）や `static` の名前も、モジュールの中で一意にする**。unity build は同じモジュールの `.cpp` をまとめてコンパイルするので、別のファイルに同じ名前があると再定義になる（2026-09-26 に `BodyFontSize` がタイトル画面とクリア画面でぶつかった）。`Scripts/Build.ps1` は `-DisableAdaptiveUnity` で常にまとめてコンパイルするので、verify で見つかる
