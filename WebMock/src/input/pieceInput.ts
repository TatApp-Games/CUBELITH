// ポインタ / タッチ入力をピース操作に変換する（RULES.md 3.3）。
...
//
// 2 本指の割り当て（解釈）: RULES.md 3.3 は回転も奥行き移動も「2 本指スワイプ **または** UI ボタン」を
...
export type PieceInputOptions = {
   ...
   * マグネットスナップ（RULES.md 3.5）を掛けるきっかけに使う。
   */
...
export function createPieceInput(options: PieceInputOptions): PieceInput {
  ...
  const onWheel = (event: WheelEvent): void => {
    // ホイールは選択の有無によらず常にズーム（RULES.md 3.3）。OrbitCamera が効いている間
